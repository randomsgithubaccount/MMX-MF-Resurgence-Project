/// @description // NORMAL ENEMY //////
function scr_hitbox_ubuster(argument0) 
{
	var noEnemy = -1;

	with(parenemy)
	{
		if protect=1
		{
		    if (collision_rectangle(bbox_left+hitoff_xp,bbox_top+hitoff_yp,bbox_left+hitoff_xp+hitbox_xp,bbox_top+hitoff_yp+hitbox_yp,other,true,false))
		    {
				instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
				sound_effect=snd_clink; with(other)	{cristal=1; instance_destroy()}
		    }
		    else if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
		    {
				if ds_exists(other.target_damaged,ds_type_list)
				{
					enemyAlreadyDamaged = ds_list_find_index(other.target_damaged, id) != noEnemy;
				}
	
				if (!enemyAlreadyDamaged)
				{
			        enehp-=argument0
			        hurt_counter+=1
			        sound_hurt="buster"
			
					if place_meeting(x,y,other)
					{
						instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
					}
				
					ds_list_add(other.target_damaged,id);
				}
		    }
		}
		else
		{
		    if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
		    {
				if ds_exists(other.target_damaged,ds_type_list)
				{
					enemyAlreadyDamaged = ds_list_find_index(other.target_damaged, id) != noEnemy;
				}
	
				if (!enemyAlreadyDamaged)
				{
			        enehp-=argument0
			        hurt_counter+=1
			        sound_hurt="buster"
			
					if place_meeting(x,y,other)
					{
						instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
					}
				
					ds_list_add(other.target_damaged,id);
				}
		    }
		}
	}
	///// SOLID ENEMY //////

	with(parenemy_solid)
	{
		if protect=1
		{
		    if (collision_rectangle(bbox_left+hitoff_xp,bbox_top+hitoff_yp,bbox_left+hitoff_xp+hitbox_xp,bbox_top+hitoff_yp+hitbox_yp,other,true,false))
		    {
				instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
				sound_effect=snd_clink; with(other)	{cristal=1; instance_destroy()}
		    }
		    else if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
		    {
				if ds_exists(other.target_damaged,ds_type_list)
				{
					enemyAlreadyDamaged = ds_list_find_index(other.target_damaged, id) != noEnemy;
				}
	
				if (!enemyAlreadyDamaged)
				{
			        enehp-=argument0
			        hurt_counter+=1
			        sound_hurt="buster"
			
					if place_meeting(x,y,other)
					{
						instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
					}
				
					ds_list_add(other.target_damaged,id);
				}
		    }
		}
		else
		{
		    if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
		    {
				if ds_exists(other.target_damaged,ds_type_list)
				{
					enemyAlreadyDamaged = ds_list_find_index(other.target_damaged, id) != noEnemy;
				}
	
				if (!enemyAlreadyDamaged)
				{
			        enehp-=argument0
			        hurt_counter+=1
			        sound_hurt="buster"
			
					if place_meeting(x,y,other)
					{
						instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
					}
				
					ds_list_add(other.target_damaged,id);
				}
		    }
		}
	}

	///// NO SOLID ENEMY //////

	with(parEnemynohit)
	{
		if protect=1
		{
		    if (collision_rectangle(bbox_left+hitoff_xp,bbox_top+hitoff_yp,bbox_left+hitoff_xp+hitbox_xp,bbox_top+hitoff_yp+hitbox_yp,other,true,false))
		    {
				instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
				sound_effect=snd_clink; with(other)	{cristal=1; instance_destroy()}
		    }
		    else if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
		    {
				if ds_exists(other.target_damaged,ds_type_list)
				{
					enemyAlreadyDamaged = ds_list_find_index(other.target_damaged, id) != noEnemy;
				}
	
				if (!enemyAlreadyDamaged)
				{
			        enehp-=argument0
			        hurt_counter+=1
			        sound_hurt="buster"
			
					if place_meeting(x,y,other)
					{
						instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
					}
				
					ds_list_add(other.target_damaged,id);
				}
		    }
		}
		else
		{
		    if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
		    {
				if ds_exists(other.target_damaged,ds_type_list)
				{
					enemyAlreadyDamaged = ds_list_find_index(other.target_damaged, id) != noEnemy;
				}
	
				if (!enemyAlreadyDamaged)
				{
			        enehp-=argument0
			        hurt_counter+=1
			        sound_hurt="buster"
			
					if place_meeting(x,y,other)
					{
						instance_create_depth(other.x+lengthdir_x(other.shock_dist,other.image_angle),other.y+lengthdir_y(other.shock_dist,other.image_angle),depth-1,resid_ultimate)
					}
				
					ds_list_add(other.target_damaged,id);
				}
		    }
		}
	}

	///// BOSS //////

	with(bossparent)
	{
	    if protect=1 or resentido=2
	    {
	        if(collision_rectangle(bbox_left+hitoff_xp,bbox_top+hitoff_yp,bbox_left+hitoff_xp+hitbox_xp,bbox_top+hitoff_yp+hitbox_yp,other,true,false))
	        {
				instance_create_depth(other.x+lengthdir_x(other.shock_dist,image_angle),other.y+lengthdir_y(other.shock_dist,image_angle),depth-1,resid_ultimate)
	            sound_effect=snd_clink
	            a=instance_create_depth(x + random_range(-5,5),y - ((sprite_height/2) + random_range(-5,5)),depth,exp_busters)
	            a.sprite_index=spr_exp_clink
	            a.image_xscale=image_xscale
	            a.image_speed=0.5
	            with(other) {cristal=1; instance_destroy()}
	        }
	        else if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
	        {
				instance_create_depth(other.x+lengthdir_x(other.shock_dist,image_angle),other.y+lengthdir_y(other.shock_dist,image_angle),depth-1,resid_ultimate)
	            resentido=1
	            global.bosshp-=argument0
	            hurt_counter+=1
	            sound_hurt="boss"
	            if global.bosshp>0 {with(other){instance_destroy()}}
	        }
	    }
	    else
	    {
	        if(collision_rectangle(bbox_left+hitoff_x,bbox_top+hitoff_y,bbox_left+hitoff_x+hitbox_x,bbox_top+hitoff_y+hitbox_y,other,true,false))
	        {
				instance_create_depth(other.x+lengthdir_x(other.shock_dist,image_angle),other.y+lengthdir_y(other.shock_dist,image_angle),depth-1,resid_ultimate)
	            resentido=1
	            global.bosshp-=argument0
	            hurt_counter+=1
	            sound_hurt="boss"
	            if global.bosshp>0 {with(other){instance_destroy()}}
	        }
	    }
	}
}
